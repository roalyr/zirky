...
class_name RebaseStar, "res://Assets_new/UI_images/SVG/icons/rebase_star.svg"

var scenes = Position3D

func _ready():
	# ============================= Connect signals ===========================
	Signals.connect_checked("sig_exited_local_space_star", self, "is_exited_local_space_star")
	Signals.connect_checked("sig_entered_local_space_star", self, "is_entered_local_space_star")
	# =========================================================================
...
func is_entered_local_space_star(zone):
	...
	if Paths.local_space_star.has_node("Scenes"):
		var scene_names = []

		for c in Paths.local_space_star.get_node("Scenes").get_children():
			scene_names.append(c)
			
		var message = "Local space '"+Paths.local_space_star.get_name() \
	...
	Paths.local_space_star.global_transform = zone.global_transform

	# Reparent scenes from global to local space.
	zone.remove_child(scenes)
	Paths.local_space_star.add_child(scenes)
...
func is_exited_local_space_star(zone):
	...
	Paths.local_space_star.remove_child(scenes)
