...
onready var coordinates_bank = SpaceState.markers_nebulas_constellations
...
func is_fetch_markers():
	...
	for coordinates in coordinates_bank:
		# Filter out null references from the lists.

		
		# TODO: if not targetable then just leave it be for some reference page or internal wiki.
